...
using Product = Swiftrade.Core.Domain.Entities.Product;
...
namespace Swiftrade.Infrastructure.Services.Catalog
{
    public class ProductService : IProductService
    {
        ...
        public virtual async Task<IList<Relatedproduct>> GetRelatedProductsByProductIdAsync(int productId, bool showHidden = false)
        {
            ...
            var relatedProducts = await _staticCacheManager.GetAsync(_staticCacheManager.PrepareKeyForDefaultCache(SwiftradeCatalogDefaults.RelatedProductsCacheKey,
        ...
        public virtual async Task<int> GetNumberOfProductsInCategoryAsync(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager
                .PrepareKeyForDefaultCache(SwiftradeCatalogDefaults.CategoryProductsNumberCacheKey, customerRoleIds, storeId, categoryIds);
        ...
        public virtual async Task<IList<Core.Domain.Entities.Product>> GetManufacturerFeaturedProductsAsync(int manufacturerId, int storeId = 0)
        {
            ...
            var customer = await _workContext.GetCurrentCustomerAsync();
            var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(SwiftradeCatalogDefaults.ManufacturerFeaturedProductIdsKey, manufacturerId, customerRoleIds, storeId);
        ...
        public virtual async Task<IList<Core.Domain.Entities.Product>> GetCategoryFeaturedProductsAsync(int categoryId, int storeId = 0)
        {
            ...
            var customer = await _workContext.GetCurrentCustomerAsync();
            var customerRoleIds = await _customerService.GetCustomerRoleIdsAsync(customer);
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(SwiftradeCatalogDefaults.CategoryFeaturedProductsIdsKey, categoryId, customerRoleIds, storeId);
