...
using Preepex.Core.Domain.Messages;
...
namespace Preepex.Core.Application.Messages
{
    /// <summary>
    /// SMTP Builder
    /// </summary>
    public class SmtpBuilder : ISmtpBuilder
    {
        ...
        public virtual async Task<SmtpClient> BuildAsync(EmailAccount emailAccount = null)
        {
            if (emailAccount is null)
            {
                ...
                ?? throw new PreepexException("Email account could not be loaded");
            }
            ...
            {
                client.Dispose();
                throw new PreepexException(ex.Message, ex);
            }
        }
