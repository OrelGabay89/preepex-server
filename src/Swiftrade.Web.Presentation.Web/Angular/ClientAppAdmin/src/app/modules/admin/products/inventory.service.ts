...
import { InventoryBrand, InventoryCategory, InventoryLabel, InventoryPagination, InventoryProduct, InventoryTag, InventoryVendor } from './inventory.types';
...
export class InventoryService
{
    ...
    private _labels: BehaviorSubject<InventoryLabel[] | null> = new BehaviorSubject(null);
     ...
     * Getter for labels
     */
    get labels$(): Observable<InventoryLabel[]>
    {
     ...
     * Get labels
     */
    getLabels(): Observable<InventoryLabel[]>
    {
        return this._httpClient.get<InventoryLabel[]>('api/apps/ecommerce/inventory/labels').pipe(
            tap((labels) => {
    ...
    createLabel(label: InventoryLabel): Observable<InventoryLabel>
    {
        return this.labels$.pipe(
            take(1),
            switchMap(labels => this._httpClient.post<InventoryLabel>('api/apps/ecommerce/inventory/label', {label}).pipe(
                map((newLabel) => {

                    // Update the labels with the new tag
    ...
    updateLabel(id: string, label: InventoryLabel): Observable<InventoryLabel>
    {
        return this.labels$.pipe(
            take(1),
            switchMap(labels => this._httpClient.patch<InventoryLabel>('api/apps/ecommerce/inventory/label', {
                id,
                label
            }).pipe(
                map((updatedLabel) => {
    ...
    deleteLabel(id: string): Observable<boolean>
    {
        return this.labels$.pipe(
            take(1),
            switchMap(labels => this._httpClient.delete('api/apps/ecommerce/inventory/label', {params: {id}}).pipe(
                map((isDeleted: boolean) => {
                ...
                switchMap(isDeleted => this.products$.pipe(
                    ...
                    map((products) => {
                        ...
                        products.forEach((product) => {
                            ...
                            const labelIndex = product.labels.findIndex(label => label === id);

                            // If the contact has the label, remove it
                            if ( labelIndex > -1 )
                            {
                                product.labels.splice(labelIndex, 1);
                            }
                        });
